...
def evaluate(cfg: DictConfig) -> Tuple[dict, dict]:
    ...
    if cfg.auto_crop:
        ...
        for i, res in enumerate(res_dicts):
            ...
            img = np.moveaxis(re_normalize(x.cpu().numpy()), source=0, destination=-1)
        ...
        csv_dir = pathlib.Path(cfg.data_dir, cfg.dataset, f'results_{cfg.seg_type}_ROI_{cfg.perturbation}.csv')
        csv_file = open(csv_dir, 'w', newline='')
        writer = csv.writer(csv_file)
        writer.writerow(['refno', 'visno', 'ethnic', 'cra', 'dry', 'ery', 'exc', 'exu', 'lic', 'oed',
